...
import type { PrismPluginOptions, PrismPluginListType, PrismPluginType, PrismLanguagesListType } from '../../../shared';
    ...
    plugins.forEach(plugin => {
      try {
        ...
        if (plugin === 'download-button' || plugin === 'autolinker') {
          buffer = fs.readFileSync(path.resolve(__dirname, `../client/static/plugins/prism-${plugin}.js`));
        } else {
...
const writeLanguages = (languages: PrismLanguagesListType = ['css', 'python', 'java', 'javascript', 'typescript', 'less',
  ...
  fs.writeFileSync(path.resolve(__dirname, '../client/static/language/prism-languages.js'), languageStr);
}
