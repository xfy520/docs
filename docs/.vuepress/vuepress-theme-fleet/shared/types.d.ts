...
export declare enum MermaidSecurityLevel {
    Strict = "strict",
    Loose = "loose",
...
export declare enum MermaidLogLevel {
    Debug = 1,
    Info = 2,
    Warn = 3,
    Error = 4,
...
export declare type PrismPluginThemeType = 'coy' | 'dark' | 'funky' | 'okaidia' | 'solarizedlight' | 'twilight' | '';
export declare type PrismPluginListType = Array<'line-numbers' | 'show-language' | 'line-highlight' | 'data-uri-highlight' | 'remove-initial-line-feed' | 'previewers' | 'match-braces' | 'inline-color' | 'autolinker' | 'show-invisibles' | 'diff-highlight' | 'highlight-keywords' | 'treeview' | {
...
export declare type PrismPluginType = {
    'line-numbers'?: string;
...
export declare type PrismLanguagesListType = Array<'abnf' | 'actionscript' | 'ada' | 'agda' | 'al' | 'antlr4' | 'apacheconf' | 'apex' | 'apl' | 'applescript' | 'aql' | 'arduino' | 'arff' | 'armasm' | 'arturo' | 'asciidoc' | 'asm6502' | 'asmatmel' | 'aspnet' | 'autohotkey' | 'autoit' | 'avisynth' | 'avro-idl' | 'awk' | 'bash' | 'basic' | 'batch' | 'bbcode' | 'bicep' | 'birb' | 'bison' | 'bnf' | 'brainfuck' | 'brightscript' | 'bro' | 'bsl' | 'c' | 'cfscript' | 'chaiscript' | 'cil' | 'clike' | 'clojure' | 'cmake' | 'cobol' | 'coffeescript' | 'concurnas' | 'cooklang' | 'coq' | 'core' | 'cpp' | 'crystal' | 'csharp' | 'cshtml' | 'csp' | 'css' | 'csv' | 'cue' | 'cypher' | 'd' | 'dart' | 'dataweave' | 'dax' | 'dhall' | 'diff' | 'django' | 'dns-zone-file' | 'docker' | 'dot' | 'ebnf' | 'editorconfig' | 'eiffel' | 'ejs' | 'elixir' | 'elm' | 'erb' | 'erlang' | 'etlua' | 'excel-formula' | 'factor' | 'false' | 'firestore-security-rules' | 'flow' | 'fortran' | 'fsharp' | 'ftl' | 'gap' | 'gcode' | 'gdscript' | 'gedcom' | 'gettext' | 'gherkin' | 'git' | 'glsl' | 'gml' | 'gn' | 'go' | 'go-module' | 'graphql' | 'groovy' | 'haml' | 'handlebars' | 'haskell' | 'haxe' | 'hcl' | 'hlsl' | 'hoon' | 'hpkp' | 'hsts' | 'http' | 'ichigojam' | 'icon' | 'icu-message-format' | 'idris' | 'iecst' | 'ignore' | 'inform7' | 'ini' | 'io' | 'j' | 'java' | 'javadoc' | 'javadoclike' | 'javascript' | 'javastacktrace' | 'jexl' | 'jolie' | 'jq' | 'jsdoc' | 'js-extras' | 'json' | 'json5' | 'jsonp' | 'jsstacktrace' | 'js-templates' | 'jsx' | 'julia' | 'keepalived' | 'keyman' | 'kotlin' | 'kumir' | 'kusto' | 'latex' | 'latte' | 'less' | 'lilypond' | 'linker-script' | 'liquid' | 'lisp' | 'livescript' | 'llvm' | 'log' | 'lolcode' | 'lua' | 'magma' | 'makefile' | 'markdown' | 'markup' | 'markup-templating' | 'mata' | 'matlab' | 'maxscript' | 'mel' | 'mermaid' | 'mizar' | 'mongodb' | 'monkey' | 'moonscript' | 'n1ql' | 'n4js' | 'nand2tetris-hdl' | 'naniscript' | 'nasm' | 'neon' | 'nevod' | 'nginx' | 'nim' | 'nix' | 'nsis' | 'objectivec' | 'ocaml' | 'odin' | 'opencl' | 'openqasm' | 'oz' | 'parigp' | 'parser' | 'pascal' | 'pascaligo' | 'pcaxis' | 'peoplecode' | 'perl' | 'php' | 'phpdoc' | 'php-extras' | 'plant-uml' | 'plsql' | 'powerquery' | 'powershell' | 'processing' | 'prolog' | 'promql' | 'properties' | 'protobuf' | 'psl' | 'pug' | 'puppet' | 'pure' | 'purebasic' | 'purescript' | 'python' | 'q' | 'qml' | 'qore' | 'qsharp' | 'r' | 'racket' | 'reason' | 'regex' | 'rego' | 'renpy' | 'rescript' | 'rest' | 'rip' | 'roboconf' | 'robotframework' | 'ruby' | 'rust' | 'sas' | 'sass' | 'scala' | 'scheme' | 'scss' | 'shell-session' | 'smali' | 'smalltalk' | 'smarty' | 'sml' | 'solidity' | 'solution-file' | 'soy' | 'sparql' | 'splunk-spl' | 'sqf' | 'sql' | 'squirrel' | 'stan' | 'stata' | 'stylus' | 'supercollider' | 'swift' | 'systemd' | 't4-cs' | 't4-templating' | 't4-vb' | 'tap' | 'tcl' | 'textile' | 'toml' | 'tremor' | 'tsx' | 'tt2' | 'turtle' | 'twig' | 'typescript' | 'typoscript' | 'unrealscript' | 'uorazor' | 'uri' | 'v' | 'vala' | 'vbnet' | 'velocity' | 'verilog' | 'vhdl' | 'vim' | 'visual-basic' | 'warpscript' | 'wasm' | 'web-idl' | 'wiki' | 'wolfram' | 'wren' | 'xeora' | 'xml-doc' | 'xojo' | 'xquery' | 'yaml' | 'yang' | 'zig' | 'abap'>;
export {};
